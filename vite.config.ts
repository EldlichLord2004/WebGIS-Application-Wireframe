    ...
    return {
      ...
      configureServer(server: any) {
        server.middlewares.use(async (req: any, res: any, next: any) => {
          if (req.method !== 'POST' || req.url !== '/api/submit_feedback') return next();
            ...
            req.on('end', async () => {
              try {
                ...
                if (!title || !content) {
                  ...
                  res.end(JSON.stringify({ ok: false, error: 'title/content is required' }));
                  return;
                }
                ...
                res.statusCode = 500;
                res.setHeader('Content-Type', 'application/json; charset=utf-8');
                res.end(JSON.stringify({ ok: false, error: e?.message ?? 'internal_error' }));
              }
            });
          } catch (e: any) {
            res.statusCode = 500;
